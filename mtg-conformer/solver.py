...
from model import CNN, parallel_all_you_want
import os
...
class Solver(object):
    def __init__(self, data_loader, valid_loader, config):
        ...
        self.tag_list = np.load('tag_list.npy')
        ...
        elif config.subset == 'moodtheme':  # only run mood-theme
            self.num_class = 56
        ...
        # Build model
        if config.model == 'CNN':
            self.model = CNN(self.num_class)
        elif config.model == 'Transformer':
            self.model = parallel_all_you_want(self.num_class)
    ...
    def train(self):
        ...
        for epoch in range(self.n_epochs):
            ...
            for x, y, _ in self.data_loader:
                ...
                # predict
                # out, _ = self.model(x)  # Changed: Softmax
                _, out = self.model(x)
            ...
            if roc_auc > best_roc_auc:
                ...
                torch.save(self.model.state_dict(), os.path.join(self.model_save_path, str(epoch)+'_best_model.pth'))
    ...
    def _validation(self, start_t, epoch):
        ...
        self.model.eval()
        reconst_loss = nn.BCELoss()
        for x, y, _ in self.valid_loader:   # Changed x,y => x,y,_
            ctr += 1
            x = x.unsqueeze(dim=1) # Changed
            # variables to cuda
            x = self.to_var(x)
            y = self.to_var(y)
